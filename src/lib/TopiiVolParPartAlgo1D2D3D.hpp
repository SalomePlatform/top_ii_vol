...
ofstream outputWrite;
ofstream infoWrite;


outputWrite.open(*outputfile+"_"+std::to_string(mpirank)+".xyz");
infoWrite.open(*outputfile+"_"+std::to_string(mpirank)+".info");  
        ...
        for(int k = kstrat; k<kend; k++)
          {
           ...
           if(mpirank==fileindex)outputWrite << std::fixed << xx[k] << "\t" << yy[k] << "\t" << zz[k] + deltaZ << "\n";
          }
      }
      kstrat  = kend -1;
      kend   += TNPtsx[i+1] -1;
         ...
         for(int l=0; l <NpZ; l++)
           {
            ...
            localPntZ += TNPtsz[l-1] ;
            if(mpirank==fileindex)infoWrite << counter <<  "\t" <<  TNPtsx[i] << "\t" <<  TNPtsz[l] <<  "\t" <<  localPntZ << "\t" <<  l <<"\n";
           }
        }
      counter = 0;
      counter++;
